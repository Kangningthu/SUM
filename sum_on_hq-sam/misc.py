...
def masks_sample_points(masks,k=10):
    ...
    for b_i in range(len(masks)):
        ...
        idx = perm[:min(k, len(perm))]
        if len(perm) < k:
            additional_indices = torch.arange(0, k - len(perm)) % len(perm)
            idx = torch.cat((idx, additional_indices))
...
def masks_sample_points_with_weight(masks, k=10, prob_weights=None):
    """Sample points on mask
    """
    if masks.numel() == 0:
        return torch.zeros((0, 2), device=masks.device)
    ...
    for b_i in range(len(masks)):
        ...
        if prob_weights is not None:
            mask_prob_weights = torch.masked_select(prob_weights[b_i], select_mask)
            perm = torch.multinomial(mask_prob_weights, k, replacement=False)
        else:
